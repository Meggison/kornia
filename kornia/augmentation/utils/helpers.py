...
def _adapted_rsampling(
    ...
    if same_on_batch:
        # Only sample once and repeat to avoid extra compute/memory
        rsample = dist.rsample(torch.Size((1, *shape[1:])))
        if shape[0] == 1:
            return rsample
        return rsample.expand(shape)  # Faster than repeat in this context
