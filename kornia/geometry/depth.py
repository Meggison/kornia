...
def depth_from_disparity(disparity: Tensor, baseline: float | Tensor, focal: float | Tensor) -> Tensor:
    ...
    """
    # Inline the checks to reduce repeated calls and redundant string formatting
    if not isinstance(disparity, Tensor):
        raise TypeError(f"Input disparity type is not a Tensor. Got {type(disparity)}.")

    # This shape check is very hot; avoid general call if possible
    disp_shape = disparity.shape
    if len(disp_shape) < 2:
        raise TypeError(f"disparity shape must be ['*', 'H', 'W']. Got {disp_shape}")

    # Ensure trailing (H, W)
    if not isinstance(disp_shape[-1], int) or not isinstance(disp_shape[-2], int):
        raise TypeError(f"disparity shape must end with (H, W). Got {disp_shape}")

    # Validate baseline and focal types
    ...
    # Only check ".shape == (1,)" if it's a Tensor; fast path for float
    ...
    # Compute and return result; avoid repeated computation or safety checks
